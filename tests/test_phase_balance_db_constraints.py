...
def test_phase_source_check_rejects_hack(tmp_path: Path) -> None:
    """Set phase_source='HACK' must raise sqlite3.IntegrityError."""
    from tools.run_calc import ensure_migrations

    db_path = tmp_path / "phase_source_constraint.sqlite"
    ensure_migrations(db_path)
    ...
    try:
        with pytest.raises(sqlite3.IntegrityError):
            con.execute("UPDATE circuits SET phase_source = 'HACK' WHERE id = ?", (circuit_id,))
            con.commit()
    finally:
        con.close()
