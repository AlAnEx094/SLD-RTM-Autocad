...
3. На странице “РТМ → Таблица нагрузок” добавить строки `rtm_rows` (как Excel‑таблица).
...
1. Открыть щит и убедиться, что РТМ рассчитан (иначе экспорт заблокирован).
...
- **3. Щит → Настройки**
- **4. РТМ → Таблица нагрузок (Excel‑экран)**
...
### 4. РТМ → Таблица нагрузок (Excel‑экран) (`rtm_rows` + `rtm_row_calc` + `rtm_panel_calc`)
...
- **Блокировать**:
  - экспорт JSON/CSV, если нет `rtm_panel_calc` (контракт DWG payload v0.4).
